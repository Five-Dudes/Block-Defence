...
function towerStats(tower) {
  ...
  if (tower.type === "dippy") {
    ...
    return {
      ...
      syrupDamage: path2 >= 5 ? 12 : 0,
      syrupRadius: path2 >= 5 ? CELL_SIZE * 2.1 : 0,
      syrupTtl: path2 >= 5 ? 7.2 : 0,
      syrupTowerBuff: path2 >= 5 ? 1.28 : 1
    };
  }
...
function spawnSecondaryProjectiles(originX, originY, count, kind, damage, splash, rain = false) {
  for (let index = 0; index < count; index += 1) {
    ...
    projectiles.push({
      ...
      clusters: false,
      rain,
      screenShake: kind === "cluster" && rain ? 0.18 : kind === "mangoBomb" ? 0.28 : 0,
      screenShakeDuration: kind === "cluster" && rain ? 0.1 : kind === "mangoBomb" ? 0.12 : 0,
...
function explodeProjectile(projectile) {
  const shakeStrength = projectile.screenShake || (projectile.splash >= 34 ? Math.min(0.42 + projectile.splash / 120, 0.9) : 0);
  const shakeDuration = projectile.screenShakeDuration || (projectile.splash >= 34 ? 0.12 + projectile.splash / 500 : 0);
  const shieldHitTracker = createShieldHitTracker();
  if (shakeStrength > 0 && shakeDuration > 0) {
    triggerScreenShake(shakeStrength, shakeDuration);
  }
...
tutorialOverlayButton?.addEventListener("click", dismissTutorialPopup);
tutorialDismissButton?.addEventListener("click", dismissEntireTutorial);
